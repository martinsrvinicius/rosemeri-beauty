          ...
          <div
            ...
            :aria-dropeffect="
              enableDragDrop && state.currentDragItem
                ? 'move'
                : enableDateSelection && state.dateSelectionOrigin
                ? 'execute'
                : 'none'
            "
...
*/
  const defaultedShowDate = computed(
    (): Date => (props.showDate ? CalendarMath.dateOnly(props.showDate) : CalendarMath.today()),
  )
  ...
  const periodStart = computed(
    (): Date =>
      CalendarMath.beginningOfPeriod(defaultedShowDate.value, props.displayPeriodUom, props.startingDayOfWeek),
  )

  const periodEnd = computed(
    (): Date =>
      CalendarMath.addDays(
        CalendarMath.incrementPeriod(periodStart.value, props.displayPeriodUom, props.displayPeriodCount),
        -1,
      ),
  )
  ...
  const displayFirstDate = computed(
    (): Date => CalendarMath.beginningOfWeek(periodStart.value, props.startingDayOfWeek),
  )
  ...
  const monthNames = computed((): string[] =>
    CalendarMath.getFormattedMonthNames(displayLocale.value, props.monthNameFormat),
  )
  const weekdayNames = computed((): string[] =>
    CalendarMath.getFormattedWeekdayNames(displayLocale.value, props.weekdayNameFormat, props.startingDayOfWeek),
  )
  ...
  const currentPeriodStart = computed(
    (): Date => CalendarMath.beginningOfPeriod(CalendarMath.today(), props.displayPeriodUom, props.startingDayOfWeek),
  )
  const currentPeriodEnd = computed(
    (): Date =>
      CalendarMath.addDays(
        CalendarMath.incrementPeriod(currentPeriodStart.value, props.displayPeriodUom, props.displayPeriodCount),
        -1,
      ),
  )
  ...
  const periodLabel = computed((): string =>
    CalendarMath.formattedPeriod(periodStart.value, periodEnd.value, props.displayPeriodUom, monthNames.value),
  )
  ...
  const getSelectedDateRange = (d: Date): [Date, Date] =>
    d <= state.dateSelectionOrigin! ? [d, state.dateSelectionOrigin!] : [state.dateSelectionOrigin!, d]
  ...
  const findAndSortItemsInDateRange = (startDate: Date, endDate: Date): INormalizedCalendarItem[] =>
    fixedItems.value
      .filter((item) => item.endDate >= startDate && CalendarMath.dateOnly(item.startDate) <= endDate, this)
      .sort(itemComparer)

  const dayHasItems = (day: Date): boolean =>
    !!fixedItems.value.find((d) => d.endDate >= day && CalendarMath.dateOnly(d.startDate) <= day)
  ...
  const getWeekItems = (weekStart: Date): INormalizedCalendarItem[] => {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      const span = Math.min(
        7 - startOffset,
        CalendarMath.dayDiff(CalendarMath.addDays(weekStart, startOffset), ep.endDate) + 1,
      )
  ...
  const getFormattedTimeRange = (item: INormalizedCalendarItem): string => {
    const startTime =
      ...
      CalendarMath.formattedTime(item.startDate, displayLocale.value, props.timeFormatOptions) +
      '</span>'
    let endTime = ''
    if (!CalendarMath.isSameDateTime(item.startDate, item.endDate)) {
      endTime =
        '<span class="endTime">' +
        CalendarMath.formattedTime(item.endDate, displayLocale.value, props.timeFormatOptions) +
        '</span>'
    }
